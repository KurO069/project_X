...
void setElecDefault(int _id)
{
    struct electricity _consumed;
    _consumed._id = _id;
    _consumed._advance = 0;
    _consumed._due = 0;
    _consumed._prevReading = 0;

    FILE *fp = fopen("modules/db/electricity.dat", "ab");
    fwrite(&_consumed, sizeof(_consumed), 1, fp);
    fclose(fp);
}

// UPDATE ELECTRICITY READING
int updateElec(struct electricity _consumed)
{
    struct electricity _tempConsumed;
    int _dataLength = 0;
    FILE *fp = fopen("modules/db/electricity.dat", "rb");
    ...
    if (_dataLength != 0)
    {
        struct electricity _arr[_dataLength];
        ...
        FILE *fp = fopen("modules/db/electricity.dat", "wb");
        fwrite(&_arr, sizeof(struct electricity), _dataLength, fp);
        fclose(fp);
...
struct electricity getElecReading(int _id)
{
    struct electricity _consumed;
    FILE *fp = fopen("modules/db/electricity.dat", "rb");
...
int calcElec(int _id)
{
    struct electricity _consumed;
    struct electricity _prev;
    _prev = getElecReading(_id);
    int _currentReading;
